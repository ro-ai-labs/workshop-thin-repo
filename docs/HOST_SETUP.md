...
- **Mac (Homebrew):** `brew install --cask temurin@21`
- **Linux (Debian/Ubuntu):**
  ```bash
  ...
  ```
- **Windows:** Download installer from https://adoptium.net/temurin/releases/?version=21
- Maven: `brew install maven` (Mac) or `sudo apt install maven` (Linux) or download from https://maven.apache.org/download.cgi

Verify: `java --version` shows 21.x; `mvn --version` works.
...
# Demo 3: Spring Boot PR demo (generated by deliverables script)
# If you have Mihai's deliverables dir checked out, run:
bash /path/to/deliverables/demo-3-setup/create_demo_repo.sh
# Otherwise, ask Mihai for the generated repo as a zip.
```
