...
cloned repo. The Dockerfile + the committed `build-context/claude-plugins/`
make `Reopen in Container` work without `docker pull`, GHCR PATs, or any
private registry. The trade-off is ~10-15 min build time on each participant's
...
- Plugin checks — `ls build-context/claude-plugins/cache/` should have all
  7 plugin dirs; if not, re-run `scripts/prepare-build-context.sh` from a
  host that has the plugins installed.
...
1. `claude /plugin list` shows 7 plugins
...
## Step 3 — Refresh plugin cache if host plugins changed

If you've installed/updated any Claude plugins on the host since Step 1,
re-stage them before participants clone:

```bash
bash scripts/prepare-build-context.sh
git add build-context/claude-plugins
git status   # verify only intended changes
git commit -m "refresh plugin cache"
```

Otherwise skip.

## Step 4 — Export tarball backup (optional, ~5 min)
...
## Step 5 — Push thin repo to GitHub
...
## Step 6 — Notify participants (tonight)
...
- **Plugin install in the image is approximate.** The Dockerfile copies the
  committed plugin cache into the image and rebuilds native deps, but
  `installed_plugins.json` / `known_marketplaces.json` paths inside the
